...
为便于后续直接生成自动化脚本，`steps` 应尽量原子化：每个 step 只描述一个用户操作、系统操作或 API 调用，并使用稳定编号（如 `S1/S2/S3`）。不要把"打开页面、填写字段并提交"合在一个 step 里；断言和观察动作可写成独立 step，具体预期放在 `expected_result`。
...
| 步骤粒度校验 | 每个 step 尽量只包含一个操作；复合步骤必须拆分后才能进入 L2 / 自动化候选 |
...
- 不总结为不可追踪的大段文字。
- 每条事实必须有文件、页码/章节/行号。
- 启动时检查 `sources/screenshots/` 是否存在软件页面截图；若存在，提取页面名、可见字段、按钮、提示文案和状态作为 UI 证据。
- 截图只作为系统输入证据，不单独推导业务规则或预期结果。
- 识别版本、冲突、重复文档。
...
- 若存在页面截图，优先用截图校准 steps 中的页面入口、按钮/字段名称和提示文案，让 L2 用例更贴近真实界面。
- L2 用例的 steps 必须尽量原子化：每个 step 一个操作，并使用 `S1/S2/S3` 编号，方便后续映射成脚本动作。
- 禁止把猜测写成确认事实。
...
- 若 steps 引用了页面、按钮、字段或提示文案，必须能追溯到页面/API 地图、字段清单或软件页面截图。
- 同一需求多次生成结果不一致时标红。
- 对 P0/P1 需求做覆盖空洞扫描。
...
4. 若提供了软件页面截图，只能用来校准页面入口、按钮/字段名称、可见状态和提示文案；不能只凭截图推导业务规则。
5. L2 用例的 steps 必须原子化：每个 step 只包含一个操作，使用 S1/S2/S3 编号；不要出现"并且/然后/同时"串联多个动作。
6. 不要为了数量制造重复用例。
7. 输出表格字段：tc_id, req_ids, evidence_ids, feature, risk_level, test_type, precondition, test_data, steps, expected_result, design_technique, automation_hint, confidence, review_status, open_questions。
```
...
2. steps 是否引入了文档、页面/API 地图、字段清单或软件截图都未定义的页面、按钮、接口、字段？
3. 每个 step 是否只包含一个操作？复合 step 是否需要拆分？
...
ai-testcase-workbench/
  sources/
    ...
    code_refs/
    defects/
    screenshots/          # 软件页面截图：页面、表单、弹窗、错误提示、权限状态等
  parsed/
    evidence_index.jsonl
    source_conflicts.md
  requirements/
...
| 软件页面截图 | 页面布局、控件文案、字段可见性、按钮状态、提示信息、典型状态图 | 更准确的 UI steps、页面断言、提示文案校准和漏测扫描 |
| 配置与规则参数 | 担保/费率/成交原则/降幅/时间间隔的取值来源 | 数值型 oracle、计算校验 |

截图的定位是"辅助可执行化"：它能证明某个页面、字段、按钮、状态或提示文案存在，但不能单独证明业务规则成立。截图内容必须和需求、字段清单、页面/API 地图或人工确认交叉验证后，才能进入 Confirmed 的 L2 用例。
...
闸门执行时还要检查 `sources/screenshots/` 是否存在。若有截图，先建立截图清单（页面名、状态、可见字段/按钮/提示、截图文件名与哈希），再把它作为系统输入资产参与 L2 注入；若截图中出现文档未定义的字段或按钮，只登记为 open question，不直接写进 Confirmed 用例。
